...
st.sidebar.image("C:/Users/banso/Data Analyst Project/Olympics/olympic.png")
...
if user_menu == 'Overall Analysis':
    ...
    df_filtered = df.drop_duplicates(['Year', 'Sport', 'Event'])
    fig, ax = plt.subplots(figsize=(25, 25))
    heatmap_data = df_filtered.pivot_table(index='Sport', columns='Year', values='Event', aggfunc='count').fillna(
        0).astype(int)
    sns.heatmap(heatmap_data, annot=True, fmt='d',linewidths=0.5, ax=ax)
    st.pyplot(fig)
...
if user_menu == 'Country-wise Analysis':
    ...
    country_df = helper.yearwise_meda_tally(df,selected_country)
    fig = px.line(country_df, x="Year", y="Medal")
    st.title(selected_country+" Medal Tally over the Years")
    st.plotly_chart(fig)

    st.title(selected_country + " excels in the following sports")
    pt = helper.country_evetnt_heatmap(df, selected_country)
